...
test("evidence flow", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: /Evidence of your client's capital/,
      level: 1,
    }),
  ).toBeVisible();

  // Select "Bank statement" and "Share certificate" and submit
  await page.getByRole("checkbox", { name: "Bank statement" }).check();
  await page.getByRole("checkbox", { name: "Share certificate" }).check();
  await page.getByRole("button", { name: "Continue" }).click();
