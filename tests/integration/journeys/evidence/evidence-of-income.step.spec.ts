...
describe("Evidence of income step", () => {
  ...
  const client = createTestClient({
    ...
    steps: [evidenceOfIncome("evidence")],
  ...
  describe("GET /cases/evidence/evidence-of-income", () => {
    ...
    it("renders 18 checkbox options", () => {
      ...
      expect(buttons[8]).to.equal(
        "Letter from friend or family providing support",
      );
      ...
      expect(buttons[12]).to.equal(
        "Latest letter showing change in benefit amount",
      );
      expect(buttons[13]).to.equal(
        "Letter from the paying agency that shows the client is receiving the passporting benefit",
      );
      expect(buttons[14]).to.equal("Original notification letter");
      expect(buttons[15]).to.equal(
        "Letter from asylum support or the local authority that shows the client is receiving support",
      );
      expect(buttons[16]).to.equal("Latest tax credit award notice");
      expect(buttons[17]).to.equal(
        "Other recent HMRC letter confirming the amount received",
      );
    });
  });
