...
describe("Date Field Validation", () => {
  ...
  describe("validatePerson - Date Field Patterns", () => {
    it("should pass validation when no date fields are provided", async () => {
      ...
      await Promise.all(
        schema.map((validation: ValidationChain) =>
          validation.run(req as Request),
        ),
      );
      ...
      expect(
        errorArray.some(
          (error: any) =>
            ...
            error.summaryMessage.includes("Date of birth must include a month"),
        ),
      ).to.be.true;
      expect(
        errorArray.some(
          (error: any) =>
            error.summaryMessage &&
            error.summaryMessage.includes("Date of birth must include a year"),
        ),
      ).to.be.true;
    });
