...
describe('requireAuth', () => {
    ...
    it('stores req.originalUrl in session.returnTo on unauthenticated request', () => {
        const { req, res, next, session } = createMocks({ originalUrl: '/some/page' });
    ...
    describe('bypass routes', () => {
        const bypassPaths = ['/health', '/status', '/auth/signin', '/auth/callback', '/auth/some-nested/path'];
...
function createMocks(options: { originalUrl?: string; isAuthenticated?: boolean } = {}) {
