...
import { MsalError, PkceGenerationError, TokenAcquisitionError, TokenRefreshError } from "#/auth/auth.errors.js";
...
describe("EntraService", () => {
  ...
  describe("exchangeAuthCode()", () => {
    ...
    beforeEach(() => {
      ...
      msalStub.acquireTokenByCode = sinon
        .stub()
        .resolves({ account: ACCOUNT, idToken: ID_TOKEN, accessToken: ACCESS_TOKEN, expiresOn: TOKEN_EXPIRY });
    ...
    it("serializes the token cache after acquiring the token", async () => {
      ...
      (msalStub.getTokenCache as sinon.SinonStub).returns({ serialize: serializeStub });

      await service.exchangeAuthCode(AUTH_CODE, authCodeRequest);

      const acquireCallOrder = (msalStub.acquireTokenByCode as sinon.SinonStub).callCount;
      const serializeCallOrder = serializeStub.callCount;
      expect(acquireCallOrder).to.equal(1);
      expect(serializeCallOrder).to.equal(1);
      expect((msalStub.acquireTokenByCode as sinon.SinonStub).calledBefore(serializeStub)).to.be.true;
    });
    ...
    it("returns undefined tokenExpiry when MSAL returns null expiresOn", async () => {
      (msalStub.acquireTokenByCode as sinon.SinonStub).resolves({
        account: ACCOUNT, idToken: ID_TOKEN, accessToken: ACCESS_TOKEN, expiresOn: null,
      });
      const result = await service.exchangeAuthCode(AUTH_CODE, authCodeRequest) as Success<TokenExchangeResult>;
  ...
  describe("getAccessToken()", () => {
    ...
    it("returns a refreshed access token on success", async () => {
      const result = await service.getAccessToken(SERIALIZED_CACHE, ACCOUNT as any) as Success<TokenExchangeResult>;
      expect(result.error).to.be.undefined;
      expect(result.value.accessToken).to.equal(ACCESS_TOKEN);
    });
    ...
    it("passes the correct scopes to acquireTokenSilent", async () => {
      await service.getAccessToken(SERIALIZED_CACHE, ACCOUNT as any);
      const [requestArg] = (msalStub.acquireTokenSilent as sinon.SinonStub).args[0];
    ...
    it("returns a TokenRefreshError failure when acquireTokenSilent throws", async () => {
      sinon.stub(console, "error");
      (msalStub.acquireTokenSilent as sinon.SinonStub).rejects(new Error("silent failure"));

      const result = await service.getAccessToken(SERIALIZED_CACHE, ACCOUNT as any);
      expect(result.error)
        .to.be.an("error")
